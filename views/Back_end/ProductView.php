        ...
        <div class="card-body row g-3">
                    ...
                    function buildCategoryOptions($categories, $parentId = null, $prefix = '', $selectedId = null) {
                        foreach ($categories as $category) {
                            if ($category['parent_id'] == $parentId) {
                                $selected = ($selectedId == $category['maLoaiSP']) ? 'selected' : '';
                                echo '<option value="' . htmlspecialchars($category['maLoaiSP']) . '" ' . $selected . '>' . $prefix . htmlspecialchars($category['tenLoaiSP']) . '</option>';
                                buildCategoryOptions($categories, $category['maLoaiSP'], $prefix . '— ', $selectedId);
                            }
                        }
                    }
                    $selectedTypeId = isset($data['editItem']) ? $data['editItem']['maLoaiSP'] : null;
                    buildCategoryOptions($data["producttype"], null, '', $selectedTypeId);
                ?>
                </select>
                <br>
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="txt_author" class="form-control"
                       value="<?php echo isset($data['editItem']) ? $data['editItem']['author'] : ''; ?>">
            </div>

            <div class="col-md-6">
                <label class="form-label">Nhà xuất bản</label>
                <input type="text" name="txt_publisher" class="form-control"
                       value="<?php echo isset($data['editItem']) ? $data['editItem']['publisher'] : ''; ?>">
            </div>
